...
export const getDashboardData = async () => {
    try {
        ...
        const orders = await Order.find({ "products.vendor._id": vendorObjectId }).populate({ path: "user", model: User }).sort({ createdAt: -1 }).limit(5).lean()
...
export const calculateTotalOrders = async () => {
    try {
        ...
        await connectToDatabase()
        const vendorObjectId = await verify_vendor()
        const orders = await Order.find({ "products.vendor._id": vendorObjectId })
...
export const orderSummary = async () => {
    try {
        ...
        const newOrders = await Order.countDocuments({
            isNew: true,
            "products.vendor._id": vendorObjectId
        })
