...
import { getParentsAndCategories, updateProduct } from '@/lib/database/actions/vendor/products/products.actions';
...
function UpdateProductComponent({ data, setOpened }: { data: any, setOpened: any }) {
    ...
    useEffect(() => {
        console.log(data)
        setVendor(data && data?.vendor?._id)
    }, [data])
    ...
    const handleSubmit = async (values: FormValues) => {
        ...
        const updateProductHandler = async () => {
            try {
                await updateProduct(productId, vendor, { ...values, color: values.color.color }).then(res => {
                    if (res.success) {
                        alert(res.message)
                        setOpened(false)
                    }
